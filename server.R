...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$generate_analysis, {
    ...
    for (i in 1:nrow(contrasts)) {
      ...
      for (protein in unique(spectral$Protein)) {
        data_cond1 <- spectral[spectral$Condition == cond1 & spectral$Protein == protein, ]
        data_cond2 <- spectral[spectral$Condition == cond2 & spectral$Protein == protein, ]
        
        # Check if there are enough observations for t-test
        if (nrow(data_cond1) > 1 & nrow(data_cond2) > 1 & var(data_cond1$spectral_counts) > 1e-10 & var(data_cond2$spectral_counts) > 1e-10) {
          t_test <- t.test(data_cond1$spectral_counts, data_cond2$spectral_counts, var.equal = TRUE,
                           alternative = "greater")
    ...
    write.table(final_results, file = "test.txt", sep = "\t", quote = FALSE, row.names = FALSE)
